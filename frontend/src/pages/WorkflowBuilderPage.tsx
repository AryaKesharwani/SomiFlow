...
const StakingConfig = ({
  ...
  const [validators, setValidators] = useState<Array<{ name: string; address: string }>>([]);
  const [loadingValidators, setLoadingValidators] = useState(true);
  const [selectedValidatorAddress, setSelectedValidatorAddress] = useState(config.validatorAddress || "");

  // Sync validator address when config changes
  useEffect(() => {
    if (config.validatorAddress !== selectedValidatorAddress) {
      setSelectedValidatorAddress(config.validatorAddress || "");
    }
  ...
  useEffect(() => {
    const fetchValidators = async () => {
      try {
        ...
        const result = await apiClient.getSomniaValidators();
        if (result.success && result.validators) {
  ...
  // Update config when validator selection changes
  const handleValidatorChange = (address: string) => {
    setSelectedValidatorAddress(address);
    onUpdate({ ...config, validatorAddress: address });
  };

  // Find selected validator name
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        {loadingValidators ? (
          <div className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-gray-50 flex items-center gap-2">
            <svg
            ...
            <span className="text-gray-500 text-sm">Loading validators...</span>
          </div>
        ) : (
          <select
      ...
      {operation === "stake" && (
        <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg">
          <p className="text-xs text-purple-800">
            <span className="font-semibold">Preview:</span> Stake {config.amount || "..."} STT{" "}
            {selectedValidator ? `to ${selectedValidator.name}` : "on Somnia testnet"}
          </p>
        </div>
      )}
      ...
      {operation === "unstake" && (
        <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg">
          <p className="text-xs text-purple-800">
            <span className="font-semibold">Preview:</span> Unstake {config.amount || "..."} STT{" "}
            {selectedValidator ? `from ${selectedValidator.name}` : "from Somnia testnet"}
          </p>
        </div>
      )}
